        ...
        <div class="">
            ...
            <ul>
                @foreach($main_positions as $main_position)
                    <li><span>{{$main_position->city}}, {{$main_position->street}}, {{$main_position->address}}</span>
                    </li>
                    <li>
                        <span>{{$main_position->hours_of_work}}, {{trans('content.weekend')}}: {{$main_position->weekend}}</span>
                    </li>
                @endforeach
            </ul>
        </div>
            ...
            <ul>
                ...
                <li class="d-flex social">
                    @foreach($social_networks as $social_network)
                        @if($social_network->social_network_type === "instagram")
                        ...
                        @endif
                        @if($social_network->social_network_type === "facebook")
                            <a href="{{$social_network->url}}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15"
                                     viewBox="0 0 15 15">
